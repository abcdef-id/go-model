...
import (
	...
	"github.com/abcdef-id/go-model/database/seeds"
...
var seedUsage = `Usage:
  go-model seed [flags] [seederName [seederName]]

Example:
  go-model seed seedOne seedTwo
	...
	if list {
		...
		for _, name := range args {
			if seeder, ok := seeds.SeederList[name]; ok {
				...
				fmt.Println(name + " not exist. run \"go-model seed -l\" to see available seeds.")
			}
		}
	}
}
